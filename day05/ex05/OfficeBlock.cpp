...
int OfficeBlock::getAll() {
	if (!this->_intern || !this->_signer || !this->_executer)
		return (0);
	return (1);
}

int OfficeBlock::getIntern() {
	if (!this->_intern)
		return (0);
	return (1);
}

int OfficeBlock::getSigner() {
	if (!this->_signer)
		return (0);
	return (1);
}

int OfficeBlock::getExecuter(){
	if (!this->_executer)
		return (0);
	return (1);
}

Bureaucrat OfficeBlock::retSigner() {
	return (*_signer);
}

Bureaucrat OfficeBlock::retExecuter() {
	return (*_executer);
}
